...
### Lobby
- **Personalized profile dashboard** — shows your League profile icon, summoner name, rank, recent record, main role, win rate, average KDA, CS/min, and evidence-based playstyle labels
- **Champion profile** — summarizes recent games, win rate, KDA, and CS/min for your most-played champions
...
- **Match history** — recent games with champion icon, final items, result, KDA, CS/min, duration, relative timestamps, and earned performance labels
- **Expandable match rankings** — opens each match in place, preserves the overall 1–10 role-adjusted LuvvyScore ranking while grouping players by blue and red team, and explains earned performance badges on hover
- **Mode filters** — filter by All, Ranked, Swiftplay, Normal, or ARAM
- **Champion filter** — filter match history by your top 5 most-played champions
- **Win/loss stats** — win rate percentage and current win/loss streak detection, with custom and practice games excluded from every profile record and average
...
### Live Game
- **Team compositions** — compact blue and red five-card grids with champion icons and summoner names, sorted by lane (top → jungle → mid → bot → support)
- **Player ranks** — rank emblems and tier text for every player in the match
- **Player labels** — data-backed badges with hover explanations, including OTP, First Time, streaks, Tilted, High Winrate, Champion Specialist, Hot Pick, Struggles on Pick, High KDA, and Veteran
...
- **Real-time recommended build panel** — dedicated side panel with starting items, skill order, and the full build sequence (boots + 4 core items) with owned/next/future states, conic-gradient progress ring on the next item, gold-needed counter, and a "READY" pulse when affordable. The same strip is mirrored compactly in the overlay
...
LuvvyLoL Desktop is not endorsed by Riot Games and does not reflect the views or opinions of Riot Games or anyone officially involved in producing or managing Riot Games properties. Riot Games and all associated properties are trademarks or registered trademarks of Riot Games, Inc.
