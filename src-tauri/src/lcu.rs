...
pub async fn apply_runes(creds: &LcuCredentials, build: &RuneBuild) -> Result<(), String> {
    ...
    // Never delete one of the player's pages. Reuse only the page owned by this
    // app, or create it once when it does not exist yet.
    let existing_id = pages.iter()
        .find(|p| p.name == "LuvvyLoL Auto" || p.name == "QueryLoL Auto")
        .and_then(|p| p.id);
    let page = LcuRunePage {
        id: existing_id,
        name: "LuvvyLoL Auto".to_string(),
    ...
    let request = if let Some(id) = existing_id {
        client.put(lcu_url(creds, &format!("/lol-perks/v1/pages/{}", id)))
    } else {
        client.post(lcu_url(creds, "/lol-perks/v1/pages"))
    };
    let resp = request
        .header("Authorization", auth_header(&creds.password))
        .json(&page)
    ...
    if resp.status().is_success() {
        ...
        Err(format!("Failed to apply rune page: {} - {}", status, body))
    }
}

pub async fn get_swiftplay_slots(creds: &LcuCredentials) -> Result<Vec<SwiftplaySlot>, String> {
    let client = lcu_client();
    let resp = client
        .get(lcu_url(creds, "/lol-lobby/v1/lobby/members/localMember/player-slots"))
        .header("Authorization", auth_header(&creds.password))
        .send()
        .await
        .map_err(|e| format!("Failed to get Swiftplay slots: {}", e))?;

    if !resp.status().is_success() {
        return Err(format!("Swiftplay slots returned: {}", resp.status()));
    }
    resp.json().await.map_err(|e| format!("Failed to parse Swiftplay slots: {}", e))
}

/// Swiftplay stores its rune page as JSON inside the slot's `perks` string.
/// This is the same page shape used by the regular lol-perks endpoint.
pub fn swiftplay_perks_string(build: &RuneBuild) -> String {
    serde_json::json!({
        "primaryStyleId": build.primary_style_id,
        "subStyleId": build.sub_style_id,
        "selectedPerkIds": build.selected_perk_ids,
    }).to_string()
}

pub async fn apply_swiftplay_slots(
    creds: &LcuCredentials,
    slots: &[SwiftplaySlot],
) -> Result<(), String> {
    let client = lcu_client();
    let resp = client
        .put(lcu_url(creds, "/lol-lobby/v1/lobby/members/localMember/player-slots"))
        .header("Authorization", auth_header(&creds.password))
        .json(slots)
        .send()
        .await
        .map_err(|e| format!("Failed to apply Swiftplay loadouts: {}", e))?;

    if resp.status().is_success() {
        info!("Swiftplay rune and spell loadouts applied");
        Ok(())
    } else {
        let status = resp.status();
        let body = resp.text().await.unwrap_or_default();
        Err(format!("Failed to apply Swiftplay loadouts: {} - {}", status, body))
    }
}
...
pub async fn apply_item_set(
    ...
    // Preserve every player-created set and replace only this app's set for
    // the same champion. Swiftplay can therefore keep both selected champions.
    let mut item_sets = match client
        .get(&url)
        .header("Authorization", auth_header(&creds.password))
        .send()
        .await
    {
        Ok(resp) if resp.status().is_success() => resp.json::<serde_json::Value>().await.ok()
            .and_then(|v| v.get("itemSets").and_then(|sets| sets.as_array()).cloned())
            .unwrap_or_default(),
        _ => vec![],
    };
    item_sets.retain(|set| {
        let title = set.get("title").and_then(|v| v.as_str());
        let ours = title == Some("LuvvyLoL Recommended")
            || title == Some("QueryLoL Recommended");
        let same_champion = set.get("associatedChampions")
            .and_then(|v| v.as_array())
            .map(|ids| ids.iter().any(|id| id.as_i64() == Some(champion_id)))
            .unwrap_or(false);
        !(ours && same_champion)
    });
